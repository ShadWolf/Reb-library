...
search-win: layout [
	across 
    text "Find what:" return
    fi-s: field "" 300 return
    tab btn "OK" [ if 0 < length? fi-s/text [ s-text: fi-s/text t/search s-text ] unview/only search-win ]
...
do test: does [
	...
	view/new/options layout  [
		...
		mn: menu with [ 
			size: 720x20 data: compose/deep [
				...
				"Search" [
					...
					bar
					"Find Next" [ if 0 < length? s-text [ t/search-next s-text ]]
					"Find Prev" [ if 0 < length? s-text [ t/search-prev s-text ]]
					bar 
		...
		tab-panel data [
			"My Funcs"  [ origin 2x2 l: func-view 150x460 ] 
			"File"   [ ] 
		]
		t: area-tc 550x500
	][resize]
